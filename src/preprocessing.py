...
class PreprocessedMultiModalDataset(Dataset):
    ...
    def preprocess_image(self, image_path: str, is_ct: bool = False) -> np.ndarray:
        ...
        """
        # Load image
        image = cv2.imread(os.path.join(self.data_dir, image_path), cv2.IMREAD_GRAYSCALE if is_ct else cv2.IMREAD_COLOR)
        if image is None:
        ...
        # Step 4: Data Augmentation (if enabled)
        if self.apply_augmentation:
            image = self.augment_image(image)

        # Step 5: Image Resizing (224x224)
